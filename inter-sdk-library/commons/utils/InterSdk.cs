...
public class InterSdk
{
    ...
    public InterSdk(string environment, string clientId, string clientSecret, byte[] certificateBytes, string certificatePassword)
    {
        config = new Config((EnvironmentEnum)Enum.Parse(typeof(EnvironmentEnum), environment), clientId, clientSecret, certificateBytes, certificatePassword);
        Warnings = new List<string>();
        OpenLog();
        DateTime notAfter = DateTime.Now;
        bool ok = CheckCertificate(certificateBytes, certificatePassword, ref notAfter);
        if (!ok)
        {
            Warnings.Add(String.Format("Certificate near do expire. Less then {0} days. Expire in {1}.", Constants.DAYS_TO_EXPIRE, notAfter));
        }
    ...
    private bool CheckCertificate(byte[] certificadoBytes, string senha, ref DateTime notAfter)
    {
        X509Certificate2Collection certificates = new X509Certificate2Collection();
        certificates.Import(certificadoBytes, senha, X509KeyStorageFlags.PersistKeySet);
        X509Certificate2 cert = certificates[0];
        TimeSpan diff = cert.NotAfter.Subtract(DateTime.Now);
        if (diff.Days < Constants.DAYS_TO_EXPIRE)
        {
